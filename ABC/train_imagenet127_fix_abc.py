...
def main():
    ...
    def create_model(ema=False):
        import models.resnet_for_imagenet127 as models
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        test_loss, _, testclassacc = validate(test_loader, ema_model, criterion, mode='Test Stats ')

        test_acc = testclassacc.mean()
        is_best = False
...
def validate(valloader, model, criterion, mode):
    val_sample_num_per_class = []
    val_labels = np.array(valloader.dataset.targets).astype(int)
    for i in range(num_class):
        val_sample_num_per_class.append(len(val_labels[val_labels == i]))
    ...
    accperclass = np.array(accperclass) / np.array(val_sample_num_per_class)
