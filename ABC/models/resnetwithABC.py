...
def resnet50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNetEncoder:
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], **kwargs)
    # return _resnet('resnet_mini', BasicBlock, [1, 1, 1, 1], **kwargs)


def resnext50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNetEncoder:
    kwargs['groups'] = 32
...
#     'resnext': [resnext50(), 2048]
# }


class ResNet(nn.Module):
    def __init__(self, num_classes, encoder_name='resnet', pretrained=False):
        super(ResNet, self).__init__()

        if encoder_name == 'resnet':
            encoder, feat_dim = resnet50(), 2048
        elif encoder_name == 'resnext':
            encoder, feat_dim = resnext50(), 2048
        else:
            raise KeyError
        ...
        if pretrained:
            ...
            part_state_dict = {k: v for k, v in state_dict.items() if k not in ['fc.weight', 'fc.bias']}
    ...
    def classify(self, out):
        return self.fc_base(out)

    def classify2(self, out):
        return self.fc_abc(out)
