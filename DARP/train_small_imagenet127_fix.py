...
parser.add_argument('--labeled_percent', type=float, default=0.01)
...
def main():
    ...
    labeled_trainloader = data.DataLoader(train_labeled_set, batch_size=args.batch_size, shuffle=True, num_workers=3,
                                          drop_last=True)
    unlabeled_trainloader = data.DataLoader(train_unlabeled_set, batch_size=args.mu * args.batch_size, shuffle=True, num_workers=3,
                                            drop_last=True)
    test_loader = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=3)
    ...
    # Resume
    title = f'Imbalanced-ImageNetLT-labeled-ratio-{args.labeled_percent}'
    if args.resume:
        # Load checkpoint.
